...
contract UnsafeLegacyUpgradesTest is Test {
    function testUUPS() public {
        ...
        address proxy = address(
            new ERC1967Proxy(
                address(new GreeterProxiable()), abi.encodeWithSelector(Greeter.initialize.selector, ("hello"))
            )
        );
        vm.stopPrank();
    ...
    function testTransparent() public {
        ...
        address proxy = address(
            new TransparentUpgradeableProxy(
                address(new Greeter()), proxyAdmin, abi.encodeWithSelector(Greeter.initialize.selector, ("hello"))
            )
        );
        vm.stopPrank();
        ...
        UnsafeUpgrades.upgradeProxy(
            proxy, address(new GreeterV2()), abi.encodeWithSelector(GreeterV2.resetGreeting.selector), msg.sender
        );
        address implAddressV2 = UnsafeUpgrades.getImplementationAddress(proxy);
    ...
    function testUpgradeProxyWithoutCaller() public {
        ...
        address proxy = address(
            new ERC1967Proxy(
                address(new GreeterProxiable()), abi.encodeWithSelector(GreeterProxiable.initialize.selector, ("hello"))
            )
        );

        UnsafeUpgrades.upgradeProxy(
            proxy, address(new GreeterV2Proxiable()), abi.encodeWithSelector(GreeterV2Proxiable.resetGreeting.selector)
        );
        vm.stopPrank();
    }
