    ...
    function run() public {
        ...
        address transparentProxy = address(
            new TransparentUpgradeableProxy(
                greeter, proxyAdmin, abi.encodeWithSelector(Greeter.initialize.selector, ("hello"))
            )
        );

        address uupsProxy = address(
            new ERC1967Proxy(greeterProxiable, abi.encodeWithSelector(GreeterProxiable.initialize.selector, ("hello")))
        );
        ...
        Upgrades.upgradeProxy(
            transparentProxy, "GreeterV2.sol", abi.encodeWithSelector(GreeterV2.resetGreeting.selector)
        );

        // example upgrade of an existing UUPS proxy
        Upgrades.upgradeProxy(
            uupsProxy, "GreeterV2Proxiable.sol", abi.encodeWithSelector(GreeterV2Proxiable.resetGreeting.selector)
        );
