...
contract UpgradesUseDefenderDeployTest is Test {
    ...
    function _assertDefenderNotAvailable(strings.slice memory slice) private pure {
        assertTrue(
            slice.contains("The current network with chainId 31337 is not supported by OpenZeppelin Defender".toSlice())
    ...
    function testDeployUUPSProxy() public {
        ...
        try d.deployUUPSProxy(
            "GreeterProxiable.sol", abi.encodeCall(GreeterProxiable.initialize, (msg.sender, "hello")), opts
        ) {
            fail();
    ...
    function testDeployTransparentProxy() public {
        ...
        try d.deployTransparentProxy(
            "Greeter.sol", msg.sender, abi.encodeCall(Greeter.initialize, (msg.sender, "hello")), opts
        ) {
            fail();
        } catch Error(string memory reason) {
            strings.slice memory slice = reason.toSlice();
    ...
    function testUpgradeProxy() public {
        ...
        try d.upgradeProxy(proxy, "GreeterV2Proxiable.sol", abi.encodeCall(GreeterV2Proxiable.resetGreeting, ()), opts)
        {
            fail();
        } catch Error(string memory reason) {
            strings.slice memory slice = reason.toSlice();
...
contract Deployer {
    function deployUUPSProxy(string memory contractName, bytes memory data, Options memory opts)
        public
        returns (address)
    {
